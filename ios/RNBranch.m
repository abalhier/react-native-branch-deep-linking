...
+ (Branch *) getInstance {
    return isTestMode ? [Branch getTestInstance] : [Branch getInstance];
}
...
+ (void)initSessionWithLaunchOptions:(NSDictionary *)launchOptions isReferrable:(BOOL)isReferrable {
    [ [RNBranch getInstance] initSessionWithLaunchOptions:launchOptions isReferrable:isReferrable andRegisterDeepLinkHandler:^(NSDictionary *params, NSError *error) {
...
+ (BOOL)handleDeepLink:(NSURL *)url {
    sourceUrl = url;
    BOOL handled = [[RNBranch getInstance] handleDeepLink:url];
    if (!handled) {
...
+ (BOOL)continueUserActivity:(NSUserActivity *)userActivity {
    return [[RNBranch getInstance] continueUserActivity:userActivity];
}
...
RCT_EXPORT_METHOD(
    ...
    [[RNBranch getInstance] getShortURLWithParams:linkPropertiesMap
                                          andTags:tags
                                       andChannel:channel
                                       andFeature:feature
                                         andStage:stage
...
RCT_EXPORT_METHOD(
    ...
    [[RNBranch getInstance] loadRewardsWithCallback:^(BOOL changed, NSError *error) {
        if(!error) {
            int credits = (int)[[RNBranch getInstance] getCredits];
...
RCT_EXPORT_METHOD(
    ...
    if (bucket) {
        [[RNBranch getInstance] redeemRewards:amount forBucket:bucket callback:^(BOOL changed, NSError *error) {
            if (!error) {
        ...
        [[RNBranch getInstance] redeemRewards:amount callback:^(BOOL changed, NSError *error) {
            if (!error) {
                resolve(@{@"changed": @(changed)});
            } else {
                NSLog(@"Redeem Rewards Error: %@", [error localizedDescription]);
...
RCT_EXPORT_METHOD(
    ...
    [[RNBranch getInstance] getCreditHistoryWithCallback:^(NSArray *list, NSError *error) {
        if (!error) {
            resolve(list);
        } else {
